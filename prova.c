...
#include "audio/includes/macros.h"
...
main (int argc, char *argv[]){
	...
	/*Aloco 10 canais, se bem q nao precisamos disso tudo né kkk*/
	if(!(Mix_Init(MIX_INIT_OGG))) printf("ERRO MIX_INIT\n %s", SDL_GetError());
	Mix_AllocateChannels(3);
	/*Os diretorios dos arquivos de som*/
	char *dir = "audio/sons/0956.ogg";
	/*Inicializando sdl_mixer)
	 * frequencia: 48kHz, formato MIX_DEFAULT_FORMAT, 2 canais de saida (stereo) e chunks de 2048 bytes*/
	if (Mix_OpenAudio(22500, MIX_DEFAULT_FORMAT, 2, 512) < 0){
		fprintf(stdout, "Falha ao inicializar audio | Erro : %s", Mix_GetError());
		return -1;
	}
	
	/*Abro os dois audios*/
	m1 = Mix_LoadWAV(dir);
	/*Falhou ao abrir algum dos audios*/
	if ( !m1){
		fprintf(stdout, "Falha ao abrir audios, erro : %s", Mix_GetError());
		return -1;
	}
	...
	while(1){
		...
		/* Esse tempo de 2 segundos vai ser o tempo q o programa vai rodar
	 * Tenho que dar sleep pros arquivos serem lidos, remova esse sleep e tente ver o q dá, mano... NADA kkkk doidera.*/
	Mix_FreeChunk(m1);	
	/*finalizando a chamada do SDL_mix;*/
	Mix_CloseAudio();
	Mix_Quit();
	SDL_Quit();
